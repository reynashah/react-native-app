...
const FlashcardScreen = ({ route }) => {
  ...
  const translateText = async (text, targetLanguage) => {
    try {
      ...
      const url = 'https://google-translate1.p.rapidapi.com/language/translate/v2?' + queryString.stringify(params);
      ...
      const options = {
        ...
        headers: {
          ...
          'X-RapidAPI-Key': '38ea9d64acmshde920b4a986cd9cp145759jsnf3fe7545575e',
  ...
  useEffect(() => {
    ...
    if (selectedCategoryData) {
      const translatedFlashcards = selectedCategoryData.words.map((word, index) => ({
